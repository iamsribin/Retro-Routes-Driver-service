  ...
  implements IDriverRepository
{
  ...
  async addEarnings(data: AddEarningsRequest) {
    ...
    if (!driver || !driver.rideDetails) throw new Error("Driver not found");

    const today = new Date();
    const startOfDay = new Date(today.setHours(0, 0, 0, 0));
    const endOfDay = new Date(today.setHours(23, 59, 59, 999));

    const todayEarningIndex = driver.rideDetails.findIndex(
      (ride) => ride.date >= startOfDay && ride.date <= endOfDay
    );

    if (todayEarningIndex !== -1) {
      // Just increment the existing day's earning
      driver.rideDetails[todayEarningIndex].Earnings =
        (driver.rideDetails[todayEarningIndex].Earnings || 0) +
        Number(driverShare);
    } else {
      driver.rideDetails.push({
      ...
      driver.adminCommission = driver.adminCommission
      ? driver.adminCommission + Number(adminShare)
      : Number(adminShare);
