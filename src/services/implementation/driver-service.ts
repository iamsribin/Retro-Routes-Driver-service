...
export class DriverService implements IDriverService {
    ...
    async fetchDriverDocuments(
        ...
        try {
            ...
            const driverDocumentDto: DriverDocumentDTO = {
                ...
                insurance: {
                    insuranceImageUrl:
                        document.vehicleDetails.insuranceImageUrl,
                    insuranceStartDate:
                        document.vehicleDetails.insuranceStartDate,
                    insuranceExpiryDate:
                        document.vehicleDetails.insuranceExpiryDate,
                },
                pollution: {
                    pollutionImageUrl:
                        document.vehicleDetails.pollutionImageUrl,
                    pollutionStartDate:
                        document.vehicleDetails.pollutionStartDate,
                    pollutionExpiryDate:
                        document.vehicleDetails.pollutionExpiryDate,
                },
            };
    ...
    async handleOnlineChange(
        ...
        try {
            ...
            if (!data.online && data.onlineTimestamp) {
                ...
                const hours =
                    Math.round((onlineDurationMs / (1000 * 60 * 60)) * 100) /
                    100;
            ...
            if (data.online) {
                ...
                await redisService.addDriverGeo(
                    data.driverId,
                    data.location.lng,
                    data.location.lat
                );
    ...
    async increaseCancelCount(payload: increaseCancelCountReq): Promise<void> {
        try {
            ...
            console.log(
                `✅ Cancel count increased for driver ${payload.driverId}`
            );
        } catch (error) {
